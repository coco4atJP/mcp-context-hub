...
npm test
npm pack
npm install --global ./nekon-mcp-context-hub-0.6.0.tgz
...
ブラウザでローカル管理画面を開きます。画面右上の「小窓で開く」をクリックすると、幅540pxのコンパクトな独立ウィンドウとして操作できます。サーバーのON/OFF切り替え、追加・削除、Skill添付、共有フォルダーの接続や版の承認、14項目の安全性スイッチなどをGUIから設定可能です。CLIと同一の設定ファイルを参照し、GUI操作の段階でMCPサーバーが起動することはありません。
...
| `add` / `update` | URL・テンプレート・許可されたローカルコマンドを登録／完全置換 |
...
| `agents` | 選択したグローバルSkills・設定の状態確認、内容確認、公開・適用 |
...
標準設定では、ローカルMCPや認証情報が必要なMCPは、所有者が `templates` に起動定義を事前登録します。エージェントはテンプレートIDを指定することで、安全にサーバーを自律追加できます。
...
テンプレートには通常のサーバー設定と同じく `env`・`allowedTools`・`skills`・`toolSkills` などを指定できます。エージェントが追加時に変更できるのは、説明・タグ・初期ON/OFF・Skill ID・ツール許可リストのみです。デフォルトでは、テンプレートで定義されたツール許可リストを拡大する変更は拒否されます。テンプレートの起動定義と権限ルールは所有者が管理します。v0.6では `allowAgentStdio`・`allowAgentCredentials`・`allowAgentSkills` の許可により、別途ローカルコマンド・認証設定・SkillパスをAgent自身で登録できます。[フルアクセスと登録例](docs/global-agents.md#agentに任せる範囲)なお、テンプレートで許可リストが省略されている場合は、そのMCPの全ツールが利用可能になります。
...
同じ設定ファイルを参照するHubは、次回のツール要求時に登録内容を同期します。新たに追加されたMCPは再起動なしで認識され、削除された接続先はその時点で切断されます。保存されたエージェント登録も、読み込み時に所有者の最新ポリシーに基づいて検証されます。権限に合わなくなった登録は停止状態で表示し、その登録だけを利用不可にします。他の登録は使い続けられます。
...
v0.6ではGUIの「同期」→「グローバルSkills・設定」で、`~/.agents/skills` 全体または選択したSkillsと、`AGENTS.md` 等の設定ファイルを共有できます。画像・補助ファイルも転送し、ローカル編集との競合は保留します。`.skill-lock.json` と安全性設定は端末側に保持します。[対象指定・CLI・バックアップ](docs/global-agents.md)
...
共有先へのエージェントによる公開許可と、受信した版の承認要求は、それぞれ独立したスイッチとして管理されます。HTTP／stdioの個別許可、HTTPS必須化、プライベートIP制限、ツール許可リストの強制、環境変数の継承なども個別に設定可能です。各端末の所有者が設定を変更でき、次回のMCPリクエスト時に反映されます。GUIの「フルアクセス」または `mcp-context-hub security preset full` で、Hub内の登録・起動・共有・受信版適用をまとめてAgentに任せられます。クライアントやOSの承認設定は別に適用されます。各設定の初期値と動作の詳細は [安全性設定](docs/security.md) を参照してください。
