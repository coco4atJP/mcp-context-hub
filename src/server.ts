...
export function createServer(initialHub: Hub, reload?: () => Promise<Hub>) {
  async function safely(action: (hub: Hub) => Promise<CallToolResult>, owner?: string, raw = false): Promise<CallToolResult> {
    try {
      const hub = reload ? await reload() : initialHub;
      await hub.refresh();
      const result = await action(hub);
  ...
  const server = new McpServer({ name: 'mcp-context-hub', version: '0.4.0' }, {
  ...
  }, async ({ query, offset, limit, server, tool }) => safely(async hub => {
  ...
  server.registerTool('hub_skill', {
    ...
    annotations: { readOnlyHint: true, openWorldHint: false },
  }, async ({ server, skill, ...options }) => safely(async hub => json(await hub.skill(server, skill, options)), server));
  ...
  }, async ({ server, ...options }, extra) => safely(async hub => json(await hub.tools(server, options, extra.signal)), server));
  ...
  }, async ({ server, tool, arguments: args }, extra) => safely(hub => hub.call(server, tool, args, extra.signal), server));
  ...
  }, async ({ server, action, options }, extra) => safely(hub => control(hub, action, server, options, extra.signal), undefined, action === 'result'));
  return server;
}
